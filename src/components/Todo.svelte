...
<li
  class=" flex flex-row items-center justify-between mb-3 py-3 px-4 md:text-xl text-md border border-gray-300"
>
  <input
    name="completed"
    class="mr-2 h-8 w-6"
  ...
  />
  <span class={`flex-1 font-bold ${todo.completed ? "line-through" : ""}`}
    >{todo.text}</span
  >
  <button
